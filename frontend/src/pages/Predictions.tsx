...
import { getLiveData, startSimulation, stopSimulation } from "../services/predictionsApi";
  ...
  const handleStartSimulation = async (): Promise<void> => {
    ...
    if (trimmedLimit.length > 0) {
      ...
      if (!Number.isFinite(parsedLimit) || parsedLimit <= 0) {
        setError("Limit must be empty or a positive integer.");
        return;
      }
      limit = parsedLimit;
    }
    ...
    try {
      const payload: SimulationControlPayload = { speed, limit };
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold tracking-tight text-[var(--text-primary)]">
                Backend Predictions Console
              </h1>
              <p className="mt-2 max-w-2xl text-sm text-[var(--text-muted)]">
                Streaming from <span className="data-mono">/live-data</span> with model outputs for t+15min, t+1hr, t+2hr and decision recommendations.
              </p>
            </div>
